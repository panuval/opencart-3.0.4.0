...
trait Common_params {
    private function _getCommonParams($address) {
        ...
        } else if (isset($this->session->data['customer']) && !empty($this->session->data['customer']['customer_group_id'])) {
            $customer_group_id = $this->session->data['customer']['customer_group_id'];
        } else if (isset($this->session->data['guest']) && !empty($this->session->data['guest']['customer_group_id'])) {
            $customer_group_id = $this->session->data['guest']['customer_group_id'];
        } 
        else if (!$customer_group_id && !empty($this->session->data['order_id'])) {
            $order_id = $this->session->data['order_id'];
            $order_info = $this->db->query("SELECT customer_group_id FROM `" . DB_PREFIX . "order` WHERE order_id = '" . (int)$order_id . "'")->row;
            if ($order_info && $order_info['customer_group_id']) {
        ...
        $payment_parts = explode('.', $payment_method);
        $payment_base = array_shift($payment_parts);
        ...
        // third party modules if available 
        if (!$coupon_code && isset($this->session->data['coupons']) && $this->session->data['coupons']) {
            $coupon_code = $this->session->data['coupons'];
        }
        ...
        $param['payment_base']      = $payment_base;
