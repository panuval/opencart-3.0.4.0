...
trait Product {
    private function getProductProfile($cart_products, $xmeta) {
       ...
       // $cart_jan = 0;
        ...
        $xdiscounted = $this->registry->has('ocmprice') && method_exists($this->ocmprice, 'getXDiscountedProducts') ? $this->ocmprice->getXDiscountedProducts(false) : array();

        // custom fields if it was set via meta
        $custom_fields = array();
        $custom_query = '';
        if (!empty($xmeta['custom_fields']) && is_array($xmeta['custom_fields'])) {
            foreach($xmeta['custom_fields'] as $custom_key) {
                if ($custom_key) {
                    $custom_fields[$custom_key] = 0;
                    $custom_fields[$custom_key . 'Total'] = 0;
                }
            }
            if ($custom_fields) {
                $xmeta['product_query'] = true;
                $custom_query = ',' . implode(',', $xmeta['custom_fields']);
            }
        }
        ...
        foreach($cart_products as $i => &$product) {
            ...
            if (!empty($product['option']) && is_array($product['option'])) {
                foreach($product['option'] as $option) {
                    if (!empty($xmeta['ignore']) && !empty($option['value']) && strpos($option['value'], $xmeta['ignore']) !== false) {
                        $skip = true;
                    } else if (!empty($xmeta['ignore']) && !empty($option['name']) && strpos($option['name'], $xmeta['ignore']) !== false) {
                        $skip = true;
                    }
                    ...
                    // xgift products
                    if (!empty($option['value']) && strpos($option['value'], '<xg>') !== false) {
                        $product['xgift'] = true; 
                    } else if (!empty($option['name']) && strpos($option['name'], '<xg>') !== false) {
                        $product['xgift'] = true; 
                    }
                }
            }
            ...
            if (!empty($xmeta['product_query'])) {
                $product_info = $this->db->query("SELECT price, manufacturer_id, location, stock_status_id $custom_query FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product['product_id'] . "'")->row;
                if ($product_info) {
                    $cart_option_price = $this->getProductOptionPrice($product['option']);
                    $cart_base_price = $product['price'] -  $cart_option_price;;
                    $product['ocm_special'] =  (float)$product_info['price'] != $cart_base_price;
                    ...
                    $product['original_tax'] = $this->tax->calculate($product['original'], $product['tax_class_id']);
                    $product['original_total'] = $product['original'] * $product['quantity'];
                    $product['original_total_tax'] = $product['original_tax'] * $product['quantity'];

                    $cart_original += $product['original_total'];
                    $cart_original_tax += $product['original_total_tax'];

                    // custom fields
                    foreach(array_keys($custom_fields) as $custom_key) {
                        if (!empty($product_info[$custom_key]) && substr($custom_key, -5) !== 'Total') {
                            $product[$custom_key] = (float)$product_info[$custom_key]; // store for future use
                            $custom_fields[$custom_key] += $product[$custom_key];
                            $custom_fields[$custom_key . 'Total'] += ($product[$custom_key] * $product['quantity']);
                        }
                    }
                }
            }
            ...
            $cart_dimension     += ($dimension * $product['quantity']);
            ...
            $product['dimension']       = $dimension * $product['quantity'];
            ...
            /* Tax Data */
            $product['tax_data'] = array();
            if ($product['tax_class_id']) {
                $tax_rates = $this->tax->getRates($product['price'], $product['tax_class_id']);
                foreach ($tax_rates as $tax_rate) {
                    if (!isset($tax_data[$tax_rate['tax_rate_id']])) $tax_data[$tax_rate['tax_rate_id']] = 0;
                    if (!isset($product['tax_data'][$tax_rate['tax_rate_id']])) $product['tax_data'][$tax_rate['tax_rate_id']] = 0;
                    $tax_data[$tax_rate['tax_rate_id']] += ($tax_rate['amount'] * $product['quantity']);
                    $product['tax_data'][$tax_rate['tax_rate_id']] += ($tax_rate['amount'] * $product['quantity']);
                }
            }
            /* packing information */
            $product['bin'] =array(
            ...
            if (!$product['price']) {
                $no_of_free++;
                $weight_of_free += $weight;
            }
            if ($product['xgift']) {
                $xgift['total'] += $product['total'];
                $xgift['weight'] += $weight;
            }
        }
        ...
        $average = $cart_sub && $cart_quantity ? ($cart_sub / $cart_quantity) : 0;
        return array(
            ...
            'no_category_specified' => count($cart_categories),
            ...
            'jan'            =>  0, // depreated - will remove soon - keeping so it trigger error for old installation
            'ean'            =>  0, // depreated - will remove soon - keeping so it trigger error for old installation
            'coupon'         => 0,
            'reward'         => $reward,
            'distance'       => 0,
            'dimensional'    => 0,
            ...
            'original_tax'          => $cart_original_tax,
            'non_shippable'         => $non_shippable,
            'tax_data'       => $tax_data,
            'stock_status'   => $cart_stock_status,
            'has_shipping'   => $has_shipping,
            'out_of_stock'   => $out_of_stock,
            'no_of_free'     => $no_of_free,
            'weight_of_free' => $weight_of_free,
    ...
    private function _adjustMultiValues(&$rules, $cart_products) {
        ...
        foreach ($possible_rules as $key) {
            if (isset($rules[$key]) 
                && ($rules[$key]['rule_type'] == 4
                    || $rules[$key]['rule_type'] == 6
                    || $rules[$key]['rule_type'] == 8
                    || $rules[$key]['rule_type'] == 9
                    || $rules[$key]['rule_type'] == 7)) {
    ...
    private function _adjustProductsOr(&$rules, $cart_products) {
        ...
        foreach ($product_rules as $key) {
            if (isset($rules[$key])) {
                $rule = $rules[$key];
                if ($rule['rule_type'] == 4 || $rule['rule_type'] == 6 || $rule['rule_type'] == 8 || $rule['rule_type'] == 9) {
                    $_products = array();
                    foreach($cart_products as $product) {
                    ...
                    if ($_products) {
                        foreach ($product_rules as $_key) {
                            ...
                            if (isset($rules[$_key])) {
                                $_rule = $rules[$_key];
                                if ($_rule['rule_type'] == 4 || $_rule['rule_type'] == 6 || $rule['rule_type'] == 8 || $rule['rule_type'] == 9) {
                                    $applied = true;
                                    $count++;
    ...
    private function _getMethodSpecificData($need_specified, $rules, $applicable_cart, $cart_data, $product_or) {
        ...
        $_method['original_tax'] = $need_specified ? 0 : $cart_data['original_tax'];
        $_method['special_tax'] = $need_specified ? 0 : $cart_data['special_tax'];
        $_method['volume'] = $need_specified ? 0 : $cart_data['volume'];
        $_method['dimension'] = $need_specified ? 0 : $cart_data['dimension'];
        ...
        $_method['no_category_specified'] = $need_specified ? 0 : $cart_data['no_category_specified'];
        ...
        $_method['jan'] = 0; // depreated - will remove soon - keeping so it trigger error for old installation
        $_method['ean'] = 0; // depreated - will remove soon
        ...
        $_method['no_of_free'] = $need_specified ? 0 : $cart_data['no_of_free'];
        $_method['weight_of_free'] = $need_specified ? 0 : $cart_data['weight_of_free'];
        ...
        $_method['xgift'] = $need_specified ? array( 'total' => 0, 'weight' => 0) : $cart_data['xgift'];
        $_method['custom_fields'] = $need_specified ? array_fill_keys(array_keys($cart_data['custom_fields']), 0) : $cart_data['custom_fields'];
        $category_specified = array();
        if ($need_specified) {
            foreach($cart_data['products'] as $product) {
                ...
                foreach ($rules as $key => $rule) {
                    ...
                    if ($key == 'category') {
                        $role_category = isset($rule['_value']) ? $rule['_value'] : $rule['value'];
                        $category_specified = array_merge($category_specified, $role_category);
                    }
                ...
                if ($count_on && !$force_off) {
                    ...
                    $_method['original']    += isset($product['original_total']) ? $product['original_total'] : 0;
                    $_method['original_tax'] += isset($product['original_total_tax']) ? $product['original_total_tax'] : 0;
                    $_method['volume']      += isset($product['volume']) ? $product['volume'] : 0;
                    $_method['dimension']   += isset($product['dimension']) ? $product['dimension'] : 0;
                    ...
                    if (!$product['price']) {
                        $_method['no_of_free']++;
                        $_method['weight_of_free'] += $product['weight'];
                    }
                    //per manufacturer
                    ...
                    if ($product['xgift']) {
                        $_method['xgift']['total'] += $product['total'];
                        $_method['xgift']['weight'] += $product['weight'];
                    }
                    // custom fields
                    foreach(array_keys($_method['custom_fields']) as $custom_key) {
                        if (!empty($product[$custom_key]) && substr($custom_key, -5) !== 'Total') {
                            $_method['custom_fields'][$custom_key] += $product[$custom_key];
                            $_method['custom_fields'][$custom_key . 'Total'] += ($product[$custom_key] * $product['quantity']);
                        }
                    }
                }  else {
            ...
            $_method['no_category_specified'] = count(array_unique($category_specified));
       }
       ...
       $_method['average'] = $_method['sub'] && $_method['quantity'] ? ($_method['sub'] / $_method['quantity']) : 0;
       return $_method;
    }
