...
final class Range extends Base {
    ...
    private function getDate($params) {
        $start = '<div class="input-group '.(VERSION < '4.0.0.0' ? 'date' : '' ).'">'
                .'<input type="text" name="' . $this->getSymbolicName($params['name'].'_start'). '" value="' . $params['preset_start'] . '" placeholder="{text_start}" data-date-format="YYYY-MM-DD" class="form-control '.(VERSION >= '4.0.0.0' ? 'date' : '' ).'" />'
                .'<div class="input-group-append input-group-addon">'
                   .'<div class="input-group-text"><i class="fa fas fa-calendar"></i></div>'
                .'</div>'
            .'</div>';

        $end = '<div class="input-group '.(VERSION < '4.0.0.0' ? 'date' : '' ).'">'
                .'<input type="text" name="' . $this->getSymbolicName($params['name'].'_end'). '" value="' . $params['preset_end'] . '" placeholder="{text_end}" data-date-format="YYYY-MM-DD" class="form-control '.(VERSION >= '4.0.0.0' ? 'date' : '' ).'" />'
                .'<div class="input-group-append input-group-addon">'
                   .'<div class="input-group-text"><i class="fa fas fa-calendar"></i></div>'
                .'</div>'
            .'</div>';
