...
final class Misc {
    ...
    private $data_attr;
    private $dir_storage;
    public function __construct($registry, $common, $meta) {
        $this->registry = $registry;
        $this->common = $common;
        $this->ocm_meta = $meta;
        $this->data_attr = VERSION >= '4.0.0.0' ? 'data-' : 'data-';
        $this->dir_storage = VERSION >= '3.0.0.0' ? DIR_STORAGE : str_replace('modification/', '', DIR_MODIFICATION); 
    }
    ...
    public function getModuleName($code, $type) {
        ...
        ob_start();
        if (VERSION >= '4.0.0.0') {
            $title = $code; // todo - if possible easily
        }
        else if (VERSION >= '3.0.0.2') {
    ...
    public function getShippingMethods($language_id, $geo_zones = array(), $inc_xshipping = true, $base_only = false) {
        ...
        if ($xshippingpro) {
            ...
            $path = $this->common->getExtPath('shipping', 'xshippingpro');
            if ($path) {
                try {
        ...
        if (file_exists($this->dir_storage . 'shipping.json')) {
            $json = @file_get_contents($this->dir_storage . 'shipping.json');
            $json_methods = @json_decode($json, true);
            if (is_array($json_methods)) {
                foreach($json_methods as $json_code => $json_method) {
                    if (in_array($json_code, $modules)) {
                        $methods[$json_code] = $json_method;
                    }    
                }  
            } else {
                $this->log->write("Invalid JSON file" . $this->dir_storage . 'shipping.json');
            }
        }
    ...
    public function getPaymentMethods($language_id, $xpayment = true) {
        ...
        if (file_exists($this->dir_storage . 'payment.json')) {
            $json = @file_get_contents($this->dir_storage . 'payment.json');
            $json_methods = @json_decode($json, true);
            if (is_array($json_methods)) {
                foreach($json_methods as $json_code => $json_name) {
                    if ($json_code && $json_name) {
                        $return[] = array(
                            'value' => $json_code,
                            'name' => ucfirst($json_name)
                        );
                    }    
                }  
            } else {
                $this->log->write("Invalid JSON file" . $this->dir_storage . 'payment.json');
            }
        }
    ...
    public function getTabs($id, $tabs) {
        ...
        foreach ($tabs as $ref => $name) {
            $markup .= '<li class="nav-item' . $active . '">
                          <a href="#' . $ref . '" class="nav-link' . $active . '" ' . $this->data_attr . 'toggle="tab">' . $name . '</a>
                       </li>';
            $active = '';
        }
        $markup .= '</ul>';
    ...
    public function getButton($params) {
        ...
        $help = isset($params['help']) ? $this->data_attr . 'toggle="tooltip" ' . $this->data_attr . 'original-title="'.$params['help'].'"' : '';
    ...
    public function getTableSkeleton($headings, $body = '__TBODY__', $footer = '', $classes = array()) {
        ...
        foreach($headings as $heading) {
            $title = isset($heading['help']) ? '<span ' . $this->data_attr . 'html="true" ' . $this->data_attr . 'toggle="tooltip" title="'.$heading['help'].'">'.$heading['title'].'</span>' : $heading['title'];
    ...
    public function getAPIKey() {
        ...
        $api_token = '';
        if (VERSION >= '4.0.0.0') {
            $api_token = '&user_token=' . $this->session->data['user_token']; // OC v4 does not need token, it uses user_token 
        }
    ...
    public function getMenuIndex($menu_id, $menus) {
        $index = -1;
        if ($menu_id) {
            foreach($menus as $i => $menu) {
                if ($menu['id'] == $menu_id && count($menu['children'])) {
                    $index = $i;
                    break;
                }
            }
