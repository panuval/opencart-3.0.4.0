...
final class Front {
    ...
    private $modules = array(
        'cms'          => array('type' => 'module', 'flag' => 'order'),
        ...
        'xgift'        => array('type' => 'module', 'flag' => 'script,shortcode,order'),
        ...
        'ordermessage' => array('type' => 'module', 'flag' => 'order'),
    ...
    public function getModel($name, $type, $domain = '') {
        $namespace = $domain ? $domain : $name;
        $path = $this->common->getExtPath($type, $namespace);
    ...
    public function getTotals($taxes, $total_mods = array()) {
        ...
        if (!$total_mods) {
            ...
            if (VERSION >= '4.0.0.0') {
                $total_mods = $extension->getExtensionsByType('total');
            } else {
        ...
        foreach ($total_mods as $total_mod) {
            ...
            if ($this->getConfig($code . '_status', 'total')) {
                $domain = !empty($total_mod['extension']) ? $total_mod['extension'] : '';
                $ext_key = $this->getModel($code, 'total', $domain);
                if (VERSION >= '4.0.0.0') {
                    // __call magic method cannot pass-by-reference so we get PHP to call it as an anonymous function.
                    ($this->{$ext_key}->getTotal)($totals, $taxes, $total);
                }
    ...
    public function isCheckoutPage() {
        $this->route = $this->getRoute();
        return !!preg_match('/.*checkout.*\/.*checkout.*|qnec|compraexpress/', $this->route);
    }
    ...
    public function parseShortcode($str, $module_only = false) {
        $return = array();
        if ($module_only) {
            $module_regex = implode('|' , array_keys($this->modules)); // add module list to omit infinity backtrack
            $rex = '/\[('.$module_regex.')\s?([^]]*)\](.*?)\[\/\w+\]/m';
        } else {
            $rex = '/\[(\w+)\s?([^]]*)\](.*?)\[\/\w+\]/m';
        }
    ...
    public function applyShortcode(&$str) {
        if ($str && $shortcodes = $this->parseShortcode($str, true)) {
            foreach($shortcodes as $shortcode) {
                ...
                if ($module && strpos($module['flag'], 'shortcode') !== false && $this->getConfig($shortcode['name'] . '_status', $module['type'])) {
                    $path = $this->common->getExtPath($module['type'], $shortcode['name']);
    ...
    public function onOrderHistory($order_id, $order_status_id) {
        foreach ($this->modules as $name => $module) {
            if (strpos($module['flag'], 'order') !== false && $this->getConfig($name . '_status', $module['type'])) {
                $ext_key = $this->getModel($name, $module['type']);
                $method_exist = property_exists($this->{$ext_key}, 'onOrderHistory') || method_exists($this->{$ext_key}, 'onOrderHistory') || isset($this->{$ext_key}->onOrderHistory);
                if ($method_exist) {
