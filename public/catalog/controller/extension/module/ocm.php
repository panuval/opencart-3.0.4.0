...
class ControllerExtensionModuleOcm extends Controller {
    ...
    public function __construct($registry) {
        ...
        $this->ocmprice = $this->registry->has('ocmprice') ? $this->registry->get('ocmprice') : new \stdClass;
    ...
    public function onViewAfter($route, $input, &$output) {
        $short_code_applieds = $this->ocm->getCache('_ocm_shortcode');
        if (!$short_code_applieds) $short_code_applieds = array();
        if (preg_match('/(common|checkout)\/.?footer$/', $route) && (!$this->ocm->getCache('_ocm_script') || $this->ocm->getCache('_ocm_script') == $route)) {
            $output = $this->ocm->getScript() . $output;
            $this->ocm->setCache('_ocm_script', $route);
        }
        else if (is_string($output) && preg_match($this->shortcode_pattern, $route) && !in_array($route, $short_code_applieds)) {
            $this->ocm->applyShortcode($output);
            $short_code_applieds[] = $route;
            $this->ocm->setCache('_ocm_shortcode', $short_code_applieds);
        }
    }
    public function onProductAfter($route, $input, &$output) {
        if (is_array($output) && $output) {
            // price modificaiton 
            if ($this->ocmprice && !is_a($this->ocmprice, 'stdClass')) {
        ...
        if ($output && is_array($output)) {
            if ($type === 'payment' || $type === 'shipping') {
                ...
                foreach($output as $i => $row) {
                    ...
                    /* discard xpayment* */
                    if (VERSION >= '4.0.0.0' && preg_match('/xpayment\d+/m', $row['code'])) {
                        unset($output[$i]);
                    }
                }
    ...
    public function onStartup() {
        /* override defautl cart */
        $this->registry->set('cart', new OCM\Lib\Ocmcart($this->registry));
    }
    /* Utility */
