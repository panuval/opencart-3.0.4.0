...
class ModelExtensionShippingXshippingpro extends Model {
    ...
    function getQuote($address) {
        ...
        if ($xmeta['payment_rule'] && !$compare_with['payment_method']) {
            ...
            $compare_with['payment_base'] = $compare_with['payment_method'];
        }
        ...
        foreach($xmethods as $xshippingpro) {
            ...
            /* if payment_base is found on payment_methods list, then It had better to use baseone. */
            ...
            if (!$alive_or_dead['status']) {
               ...
               if ($status) { 
                    ...
                    $name = $this->ocm->html_decode($xshippingpro['name'][$language_id]);
        ...
        foreach($xshippings['xmethods'] as $xshippingpro) {
            ...
            foreach ($xshippingpro['sub_options'] as $option_id => $single_option) {
                ...
                $method_options[$option_id] = array(
                    ...
                    'name'         => $single_option['name'][$language_id],
    ...
    private function getShippings() {
        ...
        $disable_cache = $this->ocm->getConfig('xshippingpro_disable_cache', $this->mtype);
        if (!$xshippingpro || $disable_cache) {
            $language_id = $this->config->get('config_language_id');
            $xmethods = array();
            $xmeta = array(
                'grand' => false,
            ...
            foreach($rows as $row) {
                ...
                if ($method_data && is_array($method_data) && $method_data['status']) {
                    ...
                    if ($method_data['category'] > 1
                        ...
                        || strpos($method_data['equation'], 'noOfCategorySpecified') !== false) {
                    ...
                    if ($method_data['manufacturer_rule'] > 1
                        ...
                        || strpos($method_data['equation'], 'special') !== false
    ...
    private function getDefaultPaymentMethod($address, $total) {
        ...
        if (VERSION >= '4.0.0.0') {
            $results = $extension->getExtensionsByType('payment');
        } else {
        ...
        foreach ($results as $result) {
            $code = $result['code'];
            $domain = !empty($result['extension']) ? $result['extension'] : '';
            $ext_key = $this->ocm->getModel($code, 'payment', $domain);
            if ($this->ocm->getConfig($code . '_status', 'payment')) {
                $ext_key = $this->ocm->getModel($code, 'payment', $domain);
                if (VERSION >= '4.0.0.0') {
                    // __call magic method cannot pass-by-reference so we get PHP to call it as an anonymous function.
                    $method = ($this->{$ext_key}->getMethods)($address, $total);
                }
                else {
                    $method = $this->{$ext_key}->getMethod($address, $total);
                }
                if ($method) {
        ...
        if ($method_data) {
            ...
            if (VERSION >= '4.0.0.0') {
                return !empty($method_data['option']) ? array_key_first($method_data['option']) : '';
            }    
            else {
    ...
    private function getJS($estimator) {
        ...
        if (VERSION >= '4.0.0.0') {
            $zoneRoute = 'localisation/country';
        } else if (VERSION >= '2.1.0.1') {
            $zoneRoute = 'extension/total/shipping/country';
        } else {
            $zoneRoute = 'checkout/shipping/country';
        }

        $url = array(
            'country' => 'index.php?route=' . $zoneRoute,
            'estimate' => 'index.php?route=extension/shipping/xshippingpro' . $this->ocm->divider . 'estimate_shipping',
            'update'   => 'index.php?route=extension/shipping/xshippingpro' . $this->ocm->divider . 'update_shipping'
        );
        ...
        if needed, use following instead of above
        foreach ($lang as $key => $value) {
            $lang[$key] = htmlentities($lang[$key], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML5, 'UTF-8');
        }
        ...
        */
        ...
        $_xshippingpro['divider'] = $this->ocm->divider;
    ...
    public function getScript() {
        ...
        if ($shipping_xshippingpro && (($this->ocm->isCheckoutPage() && !$this->ocm->isCartPage()) || ($estimator_on_store && isset($estimator['status'])))) {
            ...
            if ($shipping_req) {
                ...
                $html .= '<script src="' . $ocm_base_url . 'view/javascript/xshippingpro.min.js?v=5.0.1" defer type="text/javascript"></script>';
            }
        }
        return $html;
    }
