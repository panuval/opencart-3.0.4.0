...
;(function(window){
        ...
        this.getDom = function getDom() {
            ...
            if (this.params.title || this.params.showClose) {
                ...
                if (this.params.showClose) {
                    var closeIcon = _ocm.oc4 || (window._ocm_admin && _ocm_admin.version > '3.0.3.6') ? '&times;' : '';
    ...
    function OCMBrowser() {
        ...
        product_fitler += ' <input type="text" data-oc-target="ocm-autofill-dropdown-manufacturer_id" attr="manufacturer_id" value="" placeholder="Search By Manufacturer" class="form-control" />';
        product_fitler += ' <input type="hidden" class="manufacturer_id" name="filter[manufacturer_id]" value="0" />';
        if (_ocm.oc4) {
            product_fitler += '<ul id="ocm-autofill-dropdown-manufacturer_id" class="dropdown-menu ocm-autofill-inline"></ul>';
        }
        product_fitler += '</div>';
        product_fitler += '<div class="ocm-browser-autofill ocm-visible" ocm-attr="category_id">';
        product_fitler += ' <input type="text" data-oc-target="ocm-autofill-dropdown-category_id"  attr="category_id" value="" placeholder="Search By Category" class="form-control"/>';
        product_fitler += ' <input type="hidden" class="category_id" name="filter[category_id]" value="0" />';
        if (_ocm.oc4) {
            product_fitler += '<ul id="ocm-autofill-dropdown-category_id" class="dropdown-menu ocm-autofill-inline"></ul>';
        }
        product_fitler += '</div>';
...
function enableDragDrop() {
     ...
     $("#form-ocm").on('drop', '.draggable-container', function(e) {
          ...
          var _container = $(e.target).closest('.draggable-container')[0];
          if ($this[0] !== dragged && container === _container) { 
             var dropped = $(e.target).closest('.draggable')[0];
             if (dropped && dragged && dragged !== dropped) {
                $to_dest = $(dropped);
                $from_dest = $(dragged);
                var to_index = $to_dest.index();
                var from_index = $from_dest.index();
                if (to_index > from_index) {
                    $to_dest.after($from_dest);
                } else {
                    $to_dest.before($from_dest);
                }
             } 
          }
...
$(document).ready(function () {
    ...
    $('#ocm-container').on('click', '.ocm-more a[rel]', function(e) {
        ...
        if (window.more_help && more_help[key]) {
